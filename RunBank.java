...
public class RunBank {
        ...
        try (Scanner scan = new Scanner(new File(filename))) {
            ...
            while (scan.hasNextLine()) {
                ...
                customers.put(firstName + lastName, new Customer(idNum, firstName, lastName, dob, address, phoneNum,
                        new Checking(checkingAccNum, checkingStartBalance),
                        new Savings(savingAccNum, savingStartBalance),
                        new Credit(creditAccNum, creditMax, creditStartBalance)));
                checkingAccounts.put(checkingAccNum, new Checking(checkingAccNum, checkingStartBalance));
    ...
    private static boolean viewAccounts(String name, boolean metadata) {
        ...
        customer.accounts.forEach(account -> {
            String output = metadata ? account.toString() : "Type: " + account.getType() + ", Account Number: " + account.accNum;
    ...
    private static void TwoAccountTransaction(Scanner scan, Customer customer, Account accountOne) {
        ...
        if (!isTransfer) {
            ...
            String customerNameTwo = getUserName(scan);
        ...
        if (accountOne != null && accountTwo != null && customer != null) {
            if (!isTransfer) {
                customer.send(accountOne, accountTwo, amount);
            } else {
                customer.transfer(accountOne, accountTwo, amount);
            }
        }
    }
    ...
    private static Account getAccount(String accType, int accNum) {
        return "checking".equalsIgnoreCase(accType) ? checkingAccounts.get(accNum) :
                "savings".equalsIgnoreCase(accType) ? savingAccounts.get(accNum) : null;
    }
        ...
        for (int attempts = 0; attempts < 3; attempts++) {
            ...
            switch (input) {
                ...
                case "c":
                    ...
                    account.withdraw(Double.parseDouble(scan.nextLine()));
                    System.out.println("Current Balance: " + formatter.format(account.getBalance()));
                    return;
                case "d":
